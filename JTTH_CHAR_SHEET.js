...
var check_itemmodifiers = function(c, d) {
  ...
  getSectionIDs("repeating_inventory", function(b) {
    ...
    getAttrs(a, function(g) {
      ...
      parseInt(g[c + "_dc_bonus"], 10) : 0, n = g["mortal-level"] && !isNaN(parseInt(g["mortal-level"], 10)) ? parseInt(g["mortal-level"], 10) : 0, p = 0, u = 0;
      _.each(b, function(q) {
  ...
  "-" === c.substring(0, 1) && 20 === c.length ? do_update_mortalattack([c], d) : -1 < "power agility vitality cultivation qicontrol mental all".split(" ").indexOf(c) && getSectionIDs("repeating_mortalattack", function(a) {
    if ("all" === c) {
      ...
      getAttrs(b, function(g) {
        var f = [];
        _.each(a, function(h) {
          (g["repeating_attack_" + h + "_atkattr_base"] && -1 < g["repeating_attack_" + h + "_atkattr_base"].indexOf(c) || g["repeating_attack_" + h + "_dmgattr"] && -1 < g["repeating_attack_" + h + "_dmgattr"].indexOf(c) || g["repeating_attack_" + h + "_dmg2attr"] && -1 < g["repeating_attack_" + h + "_dmg2attr"].indexOf(c) || g["repeating_attack_" + h + "_dmg3attr"] && -1 < g["repeating_attack_" + h + "_dmg3attr"].indexOf(c) || g["repeating_attack_" + h + "_savedc"] && -1 < g["repeating_attack_" + 
          h + "_savedc"].indexOf(c)) && f.push(h);
        });
  ...
  getAttrs(d, function(a) {
    _.each(c, function(b) {
      ...
      a["repeating_mortalattack_" + b + "_atkflag"] && 0 != a["repeating_mortalattack_" + b + "_atkflag"] ? (pickbase = "pick", p = "@{wtype}&{template:atk} {{mod=@{atkbonus}}} {{rname=[@{atkname}](~repeating_mortalattack_attack_dmg)}} {{r1=[[" + h + "]]}} " + r2 + h + "]]}} {{range=@{atkrange}}} {{desc=@{atk_desc}}} {{globalattack=@{global_attack_mod}}} {{globalaccattack=@{global_acc_attack}}} {{globaleffattack=@{global_eff_attack}}} @{charname_output}") : a["repeating_mortalattack_" + b + "_dmgflag"] && 0 != a["repeating_mortalattack_" + b + "_dmgflag"] ? (pickbase = "dmg", p = "@{wtype}&{template:dmg} {{rname=@{atkname}}} @{atkflag} {{range=@{atkrange}}} @{dmgflag} {{dmg1=[[" + k + w + "]]}} {{dmg1type=" + E + "}} @{dmg2flag} {{dmg2=[[" + l + w + "]]}} {{dmg2type=" + F + "}} @{dmg3flag} {{dmg3=[[" + t + w + "]]}} {{dmg3type=" + G + "}} @{saveflag} {{desc=@{atk_desc}}} {{globaldamagetype=@{mortal_global_damage_mod_type}}} @{charname_output}") : (pickbase = "empty", p = "@{wtype}&{template:dmg} {{rname=@{atkname}}} @{atkflag} {{range=@{atkrange}}} @{saveflag} {{desc=@{atk_desc}}} @{charname_output}");
  ...
  var d = "level dtype power agility vitality cultivation qicontrol mental cultivator_global_damage_mod_roll cultivator_global_damage_mod_type".split(" ");
  _.each(c, function(a) {
  ...
  getAttrs(d, function(a) {
    _.each(c, function(b) {
      ...
      var A = a["repeating_cultivatorattack_" + b + "_dmgbase"] && "" != a["repeating_cultivatorattack_" + b + "_dmgbase"] ? a["repeating_cultivatorattack_" + b + "_dmgbase"] : 0, B = a["repeating_cultivatorattack_" + b + "_dmg2base"] && "" != a["repeating_cultivatorattack_" + b + "_dmg2base"] ? a["repeating_cultivatorattack_" + b + "_dmg2base"] : 0, C = a["repeating_cultivatorattack_" + b + "_dmg3base"] && "" != a["repeating_cultivatorattack_" + b + "_dmg3base"] ? a["repeating_cultivatorattack_" + b + "_dmg3base"] : 0, x = a["repeating_cultivatorattack_" + b + "_dmgmod"] && !1 === isNaN(parseInt(a["repeating_cultivatorattack_" + b + "_dmgmod"], 10)) ? parseInt(a["repeating_cultivatorattack_" + b + "_dmgmod"], 10) : 0, y = a["repeating_cultivatorattack_" + b + "_dmg2mod"] && !1 === isNaN(parseInt(a["repeating_cultivatorattack_" + b + "_dmg2mod"], 10)) ? parseInt(a["repeating_cultivatorattack_" + b + "_dmg2mod"], 10) : 0, z = a["repeating_cultivatorattack_" + b + "_dmg3mod"] && !1 === isNaN(parseInt(a["repeating_cultivatorattack_" + b + "_dmg3mod"], 10)) ? parseInt(a["repeating_cultivatorattack_" + b + "_dmg3mod"], 10) : 0, E = a["repeating_cultivatorattack_" + b + "_dmgtype"] ? a["repeating_cultivatorattack_" + b + "_dmgtype"] + " " : "", F = a["repeating_cultivatorattack_" + b + "_dmg2type"] ? a["repeating_cultivatorattack_" + b + "_dmg2type"] + " " : "", G = a["repeating_cultivatorattack_" + b + "_dmg3type"] ? a["repeating_cultivatorattack_" + b + "_dmg3type"] + " " : "", H = a["repeating_cultivatorattack_" + b + "_atkmod"] && "" != a["repeating_cultivatorattack_" + b + "_atkmod"] ? parseInt(a["repeating_cultivatorattack_" + b + "_atkmod"], 10) : 0, w = "+" + `[[${a.cultivator_global_damage_mod_roll && "" !== a.cultivator_global_damage_mod_roll ? a.cultivator_global_damage_mod_roll : "0"}]]` + "[" + (a.cultivator_global_damage_mod_type || "") + "]";
      ...
      a["repeating_cultivatorattack_" + b + "_atkflag"] && 0 != a["repeating_cultivatorattack_" + b + "_atkflag"] ? (pickbase = "pick", p = "@{wtype}&{template:atk} {{mod=@{atkbonus}}} {{rname=[@{atkname}](~repeating_cultivatorattack_attack_dmg)}} {{r1=[[" + h + "]]}} " + r2 + h + "]]}} {{range=@{atkrange}}} {{desc=@{atk_desc}}} {{globalattack=@{cultivator_global_attack_mod}}} {{globalaccattack=@{cultivator_global_acc_attack}}} {{globaleffattack=@{cultivator_global_eff_attack}}} @{charname_output}") : a["repeating_cultivatorattack_" + b + "_dmgflag"] && 0 != a["repeating_cultivatorattack_" + b + "_dmgflag"] ? (pickbase = "dmg", p = "@{wtype}&{template:dmg} {{rname=@{atkname}}} @{atkflag} {{range=@{atkrange}}} @{dmgflag} {{dmg1=[[" + k + w + "]]}} {{dmg1type=" + E + "}} @{dmg2flag} {{dmg2=[[" + l + w + "]]}} {{dmg2type=" + F + "}} @{dmg3flag} {{dmg3=[[" + t + w + "]]}} {{dmg3type=" + G + "}} @{saveflag} {{desc=@{atk_desc}}} {{globaldamagetype=@{cultivator_global_damage_mod_type}}} @{charname_output}") : (pickbase = "empty", p = "@{wtype}&{template:dmg} {{rname=@{atkname}}} @{atkflag} {{range=@{atkrange}}} @{saveflag} {{desc=@{atk_desc}}} @{charname_output}");
  ...
  getSectionIDs("cultivatordamagemod", function(d) {
    if (d) {
      ...
      getAttrs(b, function(g) {
        ...
        var h = {cultivator_global_damage_mod_roll:"", cultivator_global_damage_mod_type:""};
        console.log("CULTIVATOR GLOBALDAMAGE");
        _.each(a, function(k) {
          "0" != k.active_flag && (k.name && "" !== k.name && (h.cultivator_global_damage_mod_roll += k.damage + "[" + k.name + "]+"), k.type && "" !== k.type && (h.cultivator_global_damage_mod_type += k.type + "/"));
        });
        h.cultivator_global_damage_mod_roll = h.cultivator_global_damage_mod_roll.replace(/\+(?=$)/, "");
